...
if num_boards_found.value == 1:
	...
	PixelsPerStripe = 4;
	bVert = 0;
	image_lib.Generate_Stripe(ImageOne.ctypes.data_as(POINTER(c_ubyte)), WFC.ctypes.data_as(POINTER(c_ubyte)), width.value, height.value, depth.value, Reference, Variable, PixelsPerStripe, bVert, RGB.value);
		...
		for i in range(0, 10):
			...
			if(retVal == -1):
				...
				if(retVal == -1):
					...
					break
			sleep(5.0) # This is in seconds. IF USING EXTERNAL TRIGGERS, SET THIS TO 0
			#slm_lib.Delete_SDK()
